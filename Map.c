...
int findDist(Map m, LocationID src, LocationID dest) {
    ...
    // fill hops[] array with -1's
    int i;
    for (i = 0; i < NUM_MAP_LOCATIONS; i++) {
        hops[i] = UNKNOWN_LOCATION;
    }

    // start the counter of number of hops
    int counter = 0;
    ...
    while (!isQEmpty(q) && !isFound) {
        VList y;
        LocationID x = removeFromQ(q);
        for (y = m->connections[x]; y != NULL; y = y->next) {

            // haven't modified this place before?
            if (hops[y->v] == UNKNOWN_LOCATION) {
                hops[y->v] = x;
                addToQ(q,y->v);
            }
