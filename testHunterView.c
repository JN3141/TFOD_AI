...
int main()
{
    ...
    assert(whereIs(hv, PLAYER_LORD_GODALMING) == SARAJEVO);
    assert(whereIs(hv, PLAYER_DR_SEWARD) == BERLIN);
    assert(whereIs(hv, PLAYER_VAN_HELSING) == SARAJEVO);
    assert(whereIs(hv, PLAYER_MINA_HARKER) == CADIZ);
    assert(whereIs(hv, PLAYER_DRACULA) == CASTLE_DRACULA);
    assert(howHealthyIs(hv, PLAYER_LORD_GODALMING) == GAME_START_HUNTER_LIFE_POINTS - 4);
    assert(howHealthyIs(hv, PLAYER_DR_SEWARD) == GAME_START_HUNTER_LIFE_POINTS);
    assert(howHealthyIs(hv, PLAYER_VAN_HELSING) == GAME_START_HUNTER_LIFE_POINTS - 6);
    assert(howHealthyIs(hv, PLAYER_MINA_HARKER) == GAME_START_HUNTER_LIFE_POINTS);
    assert(howHealthyIs(hv, PLAYER_DRACULA) == GAME_START_BLOOD_POINTS + 20 - 4);
    assert(giveMeTheRound(hv) == 6);
    assert(giveMeTheScore(hv) == GAME_START_SCORE - 6 - 26 - 6);
    printf("Passed!\n");
    disposeHunterView(hv);
