...
struct dracView {
    ...
    LocationID fakeTrail[TRAIL_SIZE];  // includes TP's etc.
};
...
// Sets dracula's trail to real places, TP's, HI's and Dn's
static void setMyFakeTrail(char *pastPlays, LocationID *fakeTrail)
{
    int i;
    for (i = 0; i < TRAIL_SIZE; i++) fakeTrail[i] = NOWHERE;
    char *end;
    for (end = pastPlays; *end != '\0'; end++) /*skip*/ ;
    end--;
    // we jump from Drac move to Drac move
    ...
    for (p = &pastPlays[32]; p < end; p += 40) {
        if (*p == '\0') break;
        LocationID realLoc;
        if      (p[1] == 'T' && p[2] == 'P') realLoc = TELEPORT;
        else if (p[1] == 'H' && p[2] == 'I') realLoc = HIDE;
        else if (p[1] == 'D' && p[2] == '1') realLoc = DOUBLE_BACK_1;
        else if (p[1] == 'D' && p[2] == '2') realLoc = DOUBLE_BACK_2;
        else if (p[1] == 'D' && p[2] == '3') realLoc = DOUBLE_BACK_3;
        else if (p[1] == 'D' && p[2] == '4') realLoc = DOUBLE_BACK_4;
        else if (p[1] == 'D' && p[2] == '5') realLoc = DOUBLE_BACK_5;
        else {
            // must be a real location
            char place[3] = { p[1], p[2], '\0' };
            realLoc = abbrevToID(place);
        ...
        // insert location at front of trail
        for (i = TRAIL_SIZE-1; i > 0; i--) fakeTrail[i] = fakeTrail[i-1];
        fakeTrail[0] = realLoc;
    }
#if 0
    fprintf(stderr,"myTrail:");
...
DracView newDracView(char *pastPlays, PlayerMessage messages[])
{
    ...
    int i;
    for (i = 0; i < TRAIL_SIZE; i++) {
        dracView->fakeTrail[i] = dracView->myTrail[i];
    }

    setMyTrail(pastPlays, dracView->myTrail);
    setMyFakeTrail(pastPlays, dracView->fakeTrail);
    return dracView;
}
...
// Fills the trail array with Dracula's last moves
void giveMeTheFakeTrail(DracView currentView,
                        LocationID fakeTrail[TRAIL_SIZE])
{

    int i;
    for (i = 0; i < TRAIL_SIZE; i++)
        fakeTrail[i] = currentView->fakeTrail[i];
}
